...
class MinMaxAlgorithm:
    def __init__(self, http_capture: ImageCapture, logger: Logger, areas: list[dict],
    ...
    @staticmethod
    def _crop_image_by_zones(image: np.array, zones: list) -> dict:
        cropped_images = {}
        for zone in zones:
    ...
    def _add_main_zone(self):
        ...
        self._zones = [
            {'zoneId': 1, 'zoneName': 'main', 'coords': [{'x1': min_x, 'x2': max_x, 'y1': min_y, 'y2': max_y}]}]
        ...
        for item_idx, item in enumerate(self._areas.copy()):
            ...
            for subarr_idx, subarr_coord in enumerate(item['coords'].copy()):
                ...
                if zone_coords is None:
                    self._logger.fatal(f"Cannot find zone with {zone_id} zone")
